...
When `AGENT_MCP_HOST=chatgpt`, the server exposes `request_user_input` and binds it to a content-addressed MCP Apps resource such as `ui://agent-vm/request-user-input/v1-<sha256-prefix>.html`. The resource URI includes a digest of the bundled UI HTML, so changing the widget changes the tool catalog identity and prevents a host from silently reusing stale UI content. The ChatGPT adapter uses the standard MCP Apps `ui/*` bridge and submits the completed form through `ui/message`, making the answers the next user turn. The tool result also contains a plain-text fallback, so the request remains understandable if the UI cannot render.
The ChatGPT view uses the MCP Apps server-tool pattern (`tools/call`) to read and submit state through two App-only tools bound to the same resource. The server durably stores each normalized request and the validated structured answers keyed by the immutable `interactionId`; a second client rendering that interaction therefore hydrates the answers and shows the disabled `Submitted` state. Re-submitting identical answers is idempotent, while a different answer set is rejected. The UI's sandbox-local `localStorage` is only a draft cache for fields and is never consulted as the submitted-state source of truth. The state file is written atomically under the configured state root with private file permissions; hosts must expose MCP Apps server-tool capability for the authoritative hydration path.
